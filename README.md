...
5. [**Case study: checking before building a fourth tool**](case-studies/2026-09-agentic-stack-scope-check.md) — the ambition to cover the complete agentic AI stack, checked against the real 2026 market before any code shipped: a confident first pick (agent identity/least-privilege auditing) turned out to be crowded (Okta, Cisco, JumpCloud, an open-source scanner called `agent-audit`) and was retracted in the same session; a feature-by-feature comparison against that scanner found real overlap (tool-poisoning, rug-pull detection) plus two real, narrower differentiators (dual quality/security scoring, Go support); a second overclaim — "nothing else does dynamic testing" — was caught and corrected the same way, once Red Hat's AgentTrust turned out to run agents at runtime too. What survives: deterministic, zero-API-cost dynamic testing of MCP servers specifically, still unmatched.
6. [**Case study: an external founder's AI agent adversarially testing my tool, twice, same week**](case-studies/2026-09-diff-against-adversarial-testing.md) — unlike every other thread in this portfolio (a maintainer fixing their own bug), this one ran the other direction: Edward Izgorodin (co-founder, Mnemoverse) used Codex to stress-test mcp-doctor's `--diff-against` flag on [the public feedback discussion](https://github.com/modelcontextprotocol/modelcontextprotocol/discussions/3322), found a real cross-registration-style false-positive, got it root-caused and shipped same-round, then found the edge of that very fix within hours and got a second same-day release. Both fixes independently reconfirmed by his own rerun, not taken on faith.
