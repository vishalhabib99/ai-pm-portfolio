...
5. [**Case study: checking before building a fourth tool**](case-studies/2026-09-agentic-stack-scope-check.md) — the ambition to cover the complete agentic AI stack, checked against the real 2026 market before any code shipped: a confident first pick (agent identity/least-privilege auditing) turned out to be crowded (Okta, Cisco, JumpCloud, an open-source scanner called `agent-audit`) and the recommendation was retracted in the same session it was made; a feature-by-feature comparison against that scanner confirmed real overlap on tool-poisoning and rug-pull detection, but also confirmed the trilogy's one still-unclaimed edge — actually launching the server and watching it behave, which nothing else found does.
