...
- One recommendation made, then found wrong and corrected in the same session, before any code was written on it — not after.
- One direct competitor (`agent-audit`) analyzed against mcp-doctor feature-by-feature: two real overlap points acknowledged (tool poisoning, rug-pull/drift), three real differentiators confirmed still standing (dual quality/security scoring, Go support, and — across the whole check — the dynamic runtime layer nothing else found actually has).
